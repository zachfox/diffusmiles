...
def noisify_batch(batch, device, noise_cdf, reverse_rates, observation_times, num_times, max_state):
    with torch.no_grad():
        ...
        sample_times = torch.from_numpy(np.random.choice(num_times,
                                                         size=(batch_size,1),
                                                          p=sampling_prob)).to(device)
        cdf_batch = noise_cdf[(sample_times+1).long(),:,batch.long()]
        u = torch.FloatTensor(batch_size, seq_length, 1).uniform_().to(device)
...
def train_model(config):
    ...
    for epoch in range(config.n_epochs):
        ...
        avg_loss = train_one_epoch(score_model, epoch, writer, optimizer, training_loader, schedule)
...
def train_epoch(model, epoch_index, tb_writer, optimizer, training_loader):
    last_loss = 0.
    running_loss = 0.
...
def loss_fn(model, data, schedule): 
    batch, batch_dX, sample_times = noisify_batch(batch, **schedule)
    y = relu(score_fn(batch, sample_times))
    loss = torch.mean(schedule['weights'][sample_times.long()]*(y - batch_dX*torch.log(y)))
    return loss


#weights = torch.from_numpy( np.exp(-observationTimes)/(1-np.exp(-observationTimes))  ).to(config.device)
...
#    loss = torch.mean(weights[sample_times.long()].reshape([config.training.batch_size,1,1,1])*(y - birthRate_batch*torch.log(y)))
#    optimizer.zero_grad()
